...
export function Player({ player, fav }: { player: PlayerState; fav: Fav }) {
  const { t, visualizer, vizStyle, subtitles: subsOn, toggleSubtitles } = useSettings();
  ...
  return (
    <div className="scroll-area mx-auto flex h-full w-full max-w-2xl flex-col gap-3 overflow-y-auto px-5 pb-4 pt-3 sm:gap-4">
      {/* Now playing header (no album art) */}
      ...
      {/* Main panel. Swipe left/right to change track. */}
      <div className="relative min-h-0 shrink-0">
        <motion.div
          className="panel relative h-[clamp(200px,40vh,460px)] overflow-hidden rounded-[28px]"
          drag="x"
          ...
          {showSubs ? (
            <>
              ...
              <div className="absolute right-3 top-3 z-20 flex items-center gap-1.5">
                {visualizer && (
                  <PanelBtn onClick={() => setShowViz((s) => !s)} active={showViz} label="equalizer">
                    <AudioLines className="h-4 w-4" strokeWidth={2.4} />
                  </PanelBtn>
                )}
                <PanelBtn onClick={toggleSubtitles} label="hide lyrics">
                  <EyeOff className="h-4 w-4" strokeWidth={2.4} />
                </PanelBtn>
                <PanelBtn onClick={() => setFullscreen(true)} label="fullscreen">
            ...
            <>
              ...
              {hasSubs && !subsOn && (
                <div className="absolute right-3 top-3 z-20">
                  <PanelBtn onClick={toggleSubtitles} label="show lyrics">
      ...
      <div className="flex shrink-0 flex-col gap-3 sm:gap-4">
        ...
        <div className="flex items-center justify-center gap-5 lg:justify-start">
          <button onClick={player.prev} aria-label={t.controls.prev} className="pressable grid h-12 w-12 place-items-center rounded-full panel">
          ...
          <button onClick={player.next} aria-label={t.controls.next} className="pressable grid h-12 w-12 place-items-center rounded-full panel">
        ...
        <div className="flex items-center justify-center gap-5 lg:justify-start">
          ...
          <UtilBtn active={false} onClick={() => setQuote(true)} label="share">
      ...
      <AnimatePresence>
        {fullscreen && showSubs && <FullscreenLyrics player={player} onClose={() => setFullscreen(false)} />}
      </AnimatePresence>
      <AnimatePresence>
...
function PanelBtn({ onClick, active, label, children }: { onClick: () => void; active?: boolean; label: string; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      aria-label={label}
      className="pressable grid h-8 w-8 place-items-center rounded-full panel-strong"
...
function UtilBtn({ active, onClick, label, children }: { active: boolean; onClick: () => void; label: string; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      aria-label={label}
      ...
      style={{ color: active ? "var(--accent)" : "var(--text-dim)", background: active ? "var(--panel-strong)" : "transparent" }}
    >
      {children}
    </button>
  );
